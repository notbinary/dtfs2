...
describe('tasks controller  / tasks helper functions', () => {
  ...
  describe('sends updated task email', () => {
    ...
    it('should send a task email if necessary', async () => {
      const salesforceTask = MOCK_TASKS[0].groupTasks.find((t) => t.title === CONSTANTS.TASKS.AIN_AND_MIA.GROUP_1.CREATE_OR_LINK_SALESFORCE);
      const deal = {
    ...
    it('should not send a task email if not necessary', async () => {
      const nonSalesforceTask = MOCK_TASKS[0].groupTasks.find((t) => t.title !== CONSTANTS.TASKS.AIN_AND_MIA.GROUP_1.CREATE_OR_LINK_SALESFORCE);
      const deal = {
        dealSnapshot: MOCK_DEAL_MIA_SUBMITTED,
        tfm: {
          history: {
